...
<div class="min-h-[calc(100vh-8rem)] flex items-center justify-center px-4 sm:px-6 lg:px-8 py-10">
    <div class="w-full max-w-sm bg-white dark:bg-zinc-900 border border-slate-200/50 dark:border-zinc-800/80 rounded-2xl p-6 sm:p-8 shadow-mui-2 hover:shadow-mui-8 transition-all flex flex-col gap-6">
        
        <!-- Header -->
        <div class="text-center">
            <h2 class="text-2xl font-extrabold text-slate-900 dark:text-zinc-50 tracking-tight">Welcome Back</h2>
            <p class="text-xs text-slate-500 dark:text-zinc-400 mt-1">Sign in to manage and save your professional invoices.</p>
        </div>
        ...
        <div class="relative flex items-center justify-center my-0.5 select-none">
            ...
            <span class="relative bg-white dark:bg-zinc-900 px-3 text-[9px] font-bold text-slate-400 dark:text-zinc-550 uppercase tracking-widest">Or login with email</span>
        </div>
        ...
        <form action="{{ route('login') }}" method="POST" class="flex flex-col gap-5">
            ...
            <div class="relative mt-2">
                <input type="email" name="email" id="email" value="{{ old('email') }}" required autofocus placeholder=" " class="peer w-full rounded-lg border border-slate-300 dark:border-zinc-800 bg-transparent px-3.5 py-2.5 text-xs text-slate-800 dark:text-zinc-100 placeholder-transparent focus:outline-none focus:ring-1 focus:ring-indigo-600 focus:border-indigo-600" />
            ...
            <div class="flex items-center">
                <input type="checkbox" name="remember" id="remember" class="h-4 w-4 rounded border-slate-300 dark:border-zinc-800 text-indigo-600 focus:ring-indigo-500/20" />
                <label for="remember" class="ml-2 text-xs text-slate-650 dark:text-zinc-400 select-none">Remember my session</label>
            </div>

            <!-- Submit -->
            <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-750 text-white font-semibold py-2.5 rounded-lg text-xs tracking-wide shadow-mui-1 hover:shadow-mui-2 active:scale-95 cursor-pointer transition-all mt-1">
                Sign In
            </button>
        </form>
        ...
        <div class="text-center text-xs border-t border-slate-100 dark:border-zinc-800/60 pt-4">
            <span class="text-slate-500 dark:text-zinc-400">Don't have an account?</span>
            <a href="{{ route('register') }}" class="font-bold text-indigo-600 hover:text-indigo-500 ml-1 transition-colors">Create one free</a>
        </div>
    </div>
</div>
@endsection
